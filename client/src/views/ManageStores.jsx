...
export default function ManageStores() {
  ...
  const fetchStores = async () => {
    try {
      const res = await axios.get("https://stock-mate-0uuk.onrender.com/stores");
...
function EditStoreModal({ open, handleClose, store, fetchStores }) {
  ...
  const handleSave = async () => {
    try {
      await axios.put(`https://stock-mate-0uuk.onrender.com/stores/${store._id}`, { name, address, cost, sell });
      fetchStores();
      handleClose();
    } catch (error) {
      console.log("ERROR", error);
 ...
 function DeleteStoreModal({ open, handleClose, store, fetchStores }) {
  const handleDelete = async () => {
    try {
      await axios.delete(`https://stock-mate-0uuk.onrender.com/stores/${store._id}`);
      fetchStores();
      handleClose();
    } catch (error) {
      console.log("ERROR", error);
